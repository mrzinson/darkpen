...
export default function AppWorkspace() {
  ...
  const LeftSidebar = () => (
    <div className="flex flex-col h-full py-6 px-4 gap-5 overflow-y-auto scrollbar-none">
      ...
      <div className="flex flex-col gap-1">
        ...
        <SideItem icon={PATHS.groups}  label={t('groups')}        active={currentView==='groups'}         onClick={() => nav('groups')} />
      </div>

      <div className="flex flex-col gap-1">
        <p className="text-white/25 text-[9px] font-black uppercase tracking-widest px-2 mb-1">AKOONKA & SETTINGS</p>
        <SideItem icon={PATHS.billing} label="Billing & Credits" active={currentView==='billing'} onClick={() => nav('billing')} />
        <SideItem icon={PATHS.usage}   label="Isticmaalka"       active={currentView==='usage'}   onClick={() => nav('usage')} />
        <SideItem icon={PATHS.settings}label={t('settings')}     active={currentView==='settings'}onClick={() => nav('settings')} />
        <SideItem icon={PATHS.about}   label="About Darkpen"     active={currentView==='about'}   onClick={() => nav('about')} />
      </div>
  ...
  const NavPanel = () => (
    <div className="flex flex-col h-full py-6 px-4 gap-4 overflow-y-auto scrollbar-none">
      ...
      {[
        ...
        { icon: PATHS.groups,label: t('groups'),     view: 'groups',    accent: '#5B8CE5' },
      ].map(item => {
  ...
  const Content = () => {
    ...
    switch (currentView) {
      ...
      case 'shukaansi':     return <ShukaansiView onOpenSidebar={() => setLeftOpen(true)} onBack={() => nav('chat')} />;
      case 'exams':         return <ExamsView onOpenPdf={(u,t,tp) => setOpenPdf({url:u,title:t,type:tp})} onOpenSidebar={() => setLeftOpen(true)} />;
      case 'quiz':          return <QuizView onOpenSidebar={() => setLeftOpen(true)} onBack={() => nav('chat')} />;
      case 'groups':        return <GroupsView onClose={() => nav('chat')} />;
      case 'profile':       return <ProfileView userData={userData} onUpdateUser={u => setUserData(u)} onClose={() => nav('chat')} />;
      case 'exam-generator':return <ExamGeneratorView userData={userData} onUpdateUser={u => setUserData(u)} onOpenPdf={(u,t,tp) => setOpenPdf({url:u,title:t,type:tp})} onClose={() => nav('chat')} />;
      case 'billing':       return <BillingView onClose={() => nav('chat')} />;
      case 'usage':         return <UsageView onClose={() => nav('chat')} onGoToBilling={() => nav('billing')} />;
