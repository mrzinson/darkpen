    ...
    try {
        ...
        if (formattedResponse.includes('+252637930329') || formattedResponse.includes('637930329')) {
            try {
                ...
                console.error('[WHATSAPP BOT] Failed to send manager contact card:', err.message);
            }
        }
        if (formattedResponse.includes('+252654810865') || formattedResponse.includes('654810865')) {
            try {
                ...
                console.error('[WHATSAPP BOT] Failed to send payment manager contact card:', err.message);
            }
        }
        if (formattedResponse.includes('+252659119779') || formattedResponse.includes('659119779')) {
            try {
                await sendContactCard(message.from, '252659119779@c.us', 'Support Team');
            } catch (err) {
                console.error('[WHATSAPP BOT] Failed to send support contact card:', err.message);
            }
        }
