...
import { Trash2, Book, Image as ImageIcon, Edit, X } from 'lucide-react';
...
export default function BooksPanel() {
  ...
  const [editForm, setEditForm] = useState({ title: '', author: '', category: 'Biology', grade: 'Form 4', country: 'Somaliland' });
  ...
  const startUpload = (formData: FormData, title: string, mode: 'upload' | 'edit', editId?: number) => {
    ...
    const url = mode === 'edit' ? `${API_URL}/admin/books/${editId}` : `${API_URL}/admin/books`;
  ...
  const handleEditClick = (book: any) => {
    ...
    setEditForm({
      title: book.title || '',
      author: book.author || '',
      category: book.category || 'Biology',
      grade: book.grade || 'Form 4',
      country: book.country || 'Somaliland'
    });
    setEditImage(null);
    setEditPdf(null);
  };
              ...
              <div key={b.id} className="list-item">
                <div className="item-info">
                  ...
                  <span className="text-muted">
                    {b.category} • {b.grade}
                    {b.country && ` • ${b.country}`}
                    {b.region_state && ` (${b.region_state})`}
                  </span>
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button className="icon-btn" onClick={() => handleEditClick(b)} style={{ color: '#0A84FF' }}>
                    <Edit size={18} />
                  </button>
        ...
        <div className="modal-backdrop">
          <div className="modal-content">
            <div className="flex-between" style={{ marginBottom: '16px' }}>
              <h3>Edit Book: {editingItem.title}</h3>
            ...
            <form onSubmit={handleEditSubmit} className="admin-form">
              ...
              <div className="file-inputs">
                ...
                <div className="file-input">
                  <label><Book size={16} /> Book PDF (Ogow: dooro kaliya hadii aad badalayso)</label>
